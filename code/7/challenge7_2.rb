...
def possibly_correct?(result, numbers)
  ...
  OPERATORS.any? do |operator|
    ...
    # p "new_result: #{new_result} operator: #{operator} number: #{number}"
...
def calculate_reverse(first, operator, second)
  ...
  when :concatenation
    ...
    return if first_int != first

    first_int.to_s[0...-second.to_s.size].to_i if first_int.to_s.end_with?(second.to_s)
  end
end
