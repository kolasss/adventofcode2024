...
(BYTES_COUNT...@bytes.size).each do |index|
  ...
  node_finish_routes = @node_routes.dig(BOUNDARIES - 1, BOUNDARIES - 1)
  if node_finish_routes && !node_finish_routes.dig(byte_new[0], byte_new[1])
    # p "skip: #{index}"
    next
  end
  ...
  # p "node_finish: #{node_finish}"
  next if node_finish

  p "index: #{index} byte: #{byte_new}"
  break
end
